...
describe('customer category admin UI wiring', () => {
  ...
  it('pages export components and cover required UX', () => {
    ...
    expect(cat).toContain('publicationStatus')
    expect(cat).toContain('eligibleForNewBind')
    expect(cat).toContain('w.workflowName} · v${w.workflowVersion')
    const wf = readFileSync(resolve(root, 'pages/WorkflowsPage.tsx'), 'utf8')
    expect(wf).toContain('createNewWorkflowVersion')
    expect(wf).toContain('Create New Version')
