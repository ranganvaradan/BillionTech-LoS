...
> All backend services use ports 8080–8085 and 8761 — no conflict.
...
| LOS Core Service | 8083 | Unlikely |
| Notification Service | 8084 | Unlikely |
| LMS Adapter | 8085 | Unlikely |
...
**Terminal 7 — LMS Adapter:**
```bash
cd los-platform
java -jar services/lms-adapter-service/target/*.jar --server.port=8085
```

### Step 5: Verify Backend Services

After ~30 seconds, check Eureka at http://localhost:8761. You should see all 6 services registered:
- API-GATEWAY
- IAM-SERVICE
- ENROLLMENT-SERVICE
- LOS-CORE-SERVICE
...
curl http://localhost:8083/actuator/health   # Core
```
